CREATE TRIGGER `logidze_before_update_on_logidze_users`
BEFORE UPDATE ON `logidze_users` FOR EACH ROW
BEGIN
    DECLARE new_j json;
    DECLARE old_j json;
    ...
    IF COALESCE(@logidze.disabled, '') <> 'on' THEN
        ...
        SET old_j = JSON_OBJECT('email', OLD.email, 'position', OLD.position, 'name', OLD.name, 'bio', OLD.bio, 'age', OLD.age, 'dump', OLD.dump, 'data', OLD.data);
        SET columns_j = JSON_ARRAY('email', 'position', 'name', 'bio', 'age', 'dump', 'data');

        SET log_data = logidze_logger(old_j, new_j, JSON_ARRAY('name', 'age', 'active', 'log_data'), 'UPDATE', NULL);
